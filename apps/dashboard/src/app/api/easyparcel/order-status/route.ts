...
export async function POST(req: NextRequest) {
  ...
  const result = await epPost(isDemo, 'MPOrderStatusBulk', {
    authentication: authKey,
    api:            apiKey,
    bulk,
  })

  // Update order_status in easyparcel_shipments table
  if (result.api_status === 'Success' && result.result) {
    for (let i = 0; i < result.result.length; i++) {
      const r = result.result[i]
      if (shipmentIds?.[i] && r.order_no) {
        await supabase.from('easyparcel_shipments').update({
          order_status: r.order_status, // "Paid", "Waiting Payment", etc.
