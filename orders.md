...
# Add Revenue tracking, order count, average order value, refunds, tax collected, shipping fees collected, order status, coupon usage, and sales by billing or shipping region.

Here is the complete Reports module — 4 files covering the SQL layer, the server page, and the full client with every metric.[^1][^2][^3]
...
```sql
-- ── New columns on orders (add only if missing) ───────────────────────────
ALTER TABLE orders
...
CREATE TABLE IF NOT EXISTS refunds (
  ...
  refund_method  text     NOT NULL DEFAULT 'manual',   -- manual|wallet|original_payment
  notes          text,
...
CREATE POLICY "merchant manages own refunds"
  ON refunds FOR ALL TO authenticated
  USING (merchant_id IN (SELECT id FROM merchants WHERE user_id = auth.uid()));

-- ── RPC: get_revenue_summary ──────────────────────────────────────────────
-- Returns every headline metric for a date range, plus comparison period
...
LANGUAGE sql STABLE AS $$
  SELECT
    ...
    COALESCE(SUM(COALESCE(discount_amount,0)
             + COALESCE(coupon_discount,0)
             + COALESCE(points_discount,0)),            0) AS total_discounts,
    COALESCE(SUM(refunded_amount),                      0) AS total_refunds,
    COALESCE(SUM(total_amount - COALESCE(delivery_fee,0)
             - COALESCE(tax_amount,0)
             - COALESCE(refunded_amount,0)),            0) AS net_revenue,
...
-- ── RPC: get_daily_revenue_range ──────────────────────────────────────────
CREATE OR REPLACE FUNCTION get_daily_revenue_range(
  p_merchant_id  uuid,
  p_start        date,
  p_end          date
)
RETURNS TABLE (date text, revenue numeric, orders bigint, shipping numeric)
LANGUAGE sql STABLE AS $$
  SELECT
    gs::date::text                          AS date,
    COALESCE(SUM(o.total_amount),      0)  AS revenue,
    COALESCE(COUNT(o.id),              0)  AS orders,
    COALESCE(SUM(o.delivery_fee),      0)  AS shipping
...
-- ── RPC: get_sales_by_region ──────────────────────────────────────────────
CREATE OR REPLACE FUNCTION get_sales_by_region(
  p_merchant_id  uuid,
  p_start        date,
  p_end          date
)
RETURNS TABLE (region text, order_count bigint, total_revenue numeric, total_shipping numeric)
LANGUAGE sql STABLE AS $$
  SELECT
    COALESCE(
      NULLIF(TRIM(delivery_address->>'state'), ''),
      'Unknown'
    )                                        AS region,
...
-- ── RPC: get_order_status_breakdown ──────────────────────────────────────
CREATE OR REPLACE FUNCTION get_order_status_breakdown(
  p_merchant_id  uuid,
  p_start        date,
  p_end          date
)
RETURNS TABLE (status text, order_count bigint, total_revenue numeric)
LANGUAGE sql STABLE AS $$
  SELECT
    status,
    COUNT(*)                       AS order_count,
...
-- ── RPC: get_coupon_usage ─────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION get_coupon_usage(
  p_merchant_id  uuid,
  p_start        date,
  p_end          date
)
...
-- ── RPC: get_refunds_summary ──────────────────────────────────────────────
CREATE OR REPLACE FUNCTION get_refunds_summary(
  p_merchant_id  uuid,
  p_start        date,
  p_end          date
)
RETURNS TABLE (
  total_refunds_count  bigint,
  total_refunded_rm    numeric,
...
LANGUAGE sql STABLE AS $$
  SELECT
    ...
    COALESCE(SUM(CASE WHEN status = 'approved' THEN amount ELSE 0 END), 0) AS total_refunded_rm,
...
## Step 2 — `src/app/(dashboard)/reports/page.tsx`
...
export default async function ReportsPage({
  ...
  const [
    ...
    supabase
      .from('refunds')
      .select('*, order:orders(order_number, total_amount), customer:customer_id(full_name)')
  ...
  const changes = {
    revenue:    pct(cur.total_revenue,    prev.total_revenue),
    orders:     pct(cur.total_orders,     prev.total_orders),
    aov:        pct(cur.avg_order_value,  prev.avg_order_value),
    shipping:   pct(cur.total_shipping,   prev.total_shipping),
    tax:        pct(cur.total_tax,        prev.total_tax),
    discounts:  pct(cur.total_discounts,  prev.total_discounts),
    refunds:    pct(cur.total_refunds,    prev.total_refunds),
    customers:  pct(cur.unique_customers, prev.unique_customers),
  }
...
## Step 3 — `src/components/dashboard/ReportsClient.tsx`
          ...
          <Section title="Revenue & Orders Over Time"
            ...
            <ResponsiveContainer width="100%" height={260}>
              <AreaChart data={revenueData} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                <defs>
                  earGradient id="rev" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%"  stopColor="#2563eb" stopOpacity={0.15} />
                    <stop offset="95%" stopColor="#2563eb" stopOpacity={0}    />
                  </linearGradient>
                  earGradient id="ship" x1="0" y1="0" x2="0" y2="1">
                ...
                <Tooltip
                  formatter={(v: number, name: string) =>
                    name === 'orders' ? [v, 'Orders'] : [`RM ${v.toFixed(2)}`, name === 'revenue' ? 'Revenue' : 'Shipping']}
...
function RefundModal({ onClose, onCreate, merchantId }: {
  ...
  const searchOrders = async (q: string) => {
    ...
    const { data } = await supabase
      .from('orders').select('id, order_number, total_amount, customer_id, profiles:customer_id(full_name)')
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4"
      ...
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl p-6 space-y-4">
        ...
        {/* Order search */}
        <div>
          abel className="text-sm font-medium text-gray-700">Order *</label>
        ...
        <div>
          abel className="text-sm font-medium text-gray-700">Refund Amount (RM) *</label>
        ...
        <div>
          abel className="text-sm font-medium text-gray-700">Reason</label>
          <Input className="mt-1" value={form.reason} onChange={e => set('reason', e.target.value)}
            placeholder="e.g. Wrong item delivered" />
        </div>

        <div>
          abel className="text-sm font-medium text-gray-700">Refund Method</label>
        ...
        <div>
          abel className="text-sm font-medium text-gray-700">Internal Notes</label>
...
| Metric | Source | Tab |
| :-- | :-- | :-- |
| Total Revenue | `SUM(total_amount)` | Overview |
| Net Revenue | Revenue − shipping − tax − refunds | Overview |
| Order Count | `COUNT(*)` | Overview / Orders |
| Avg Order Value | `AVG(total_amount)` | Overview |
| Shipping Fees Collected | `SUM(delivery_fee)` | Overview |
| Tax Collected | `SUM(tax_amount)` | Overview |
| Discounts Given | points + promo + coupon | Overview |
| Refunds Issued | `SUM(refunded_amount)` | Overview / Refunds |
| Unique Customers | `COUNT(DISTINCT customer_id)` | Overview |
| Revenue over time | `get_daily_revenue_range` RPC | Overview chart |
| Order status breakdown | `get_order_status_breakdown` RPC | Orders |
| Daily order volume | same RPC | Orders |
| Sales by state | `get_sales_by_region` RPC | Regions [^2] |
| Coupon usage + discount rate | `get_coupon_usage` RPC | Coupons |
| Refund list + approve/reject | `refunds` table | Refunds |
| Period comparison (% change) | previous period RPC call | All cards [^1] |
| CSV export | `exportCSV()` | Revenue, Regions, Coupons, Refunds |

<span style="display:none">[^10][^11][^12][^13][^14][^15][^4][^5][^6][^7][^8][^9]</span>

<div align="center">⁂</div>

[^1]: https://codewithfimi.com/14-sql-aggregation-examples-for-business-reporting/

[^2]: https://sdk.myinvois.hasil.gov.my/codes/state-codes/

[^3]: https://codevisionz.com/lessons/sql-views-ecommerce-analytics-dashboard/

[^4]: https://www.linkedin.com/posts/kavita-bijarniya-249b58260_e-commerce-sql-analysis-activity-7436961542543269888-1upj

[^5]: https://contra.com/community/bcQK7VFj-expert-revenue-analysis-using-sql-for

[^6]: https://www.quadratichq.com/recipes/ecommerce-performance-analytics

[^7]: https://www.slideshare.net/slideshow/e-shop-sql-data-analysis-business-insights-performance-metrics/285543636

[^8]: https://ambercourier.com/zone-list/

[^9]: https://www.elastic.co/docs/explore-analyze/query-filter/aggregations/tutorial-analyze-ecommerce-data-with-aggregations-using-query-dsl

[^10]: https://www.gooddata.com/blog/sql-and-aggregated-data-there-better-way/

[^11]: https://en.wikipedia.org/wiki/Postal_codes_in_Malaysia

[^12]: https://my-packs.ninjavan.co/pages/serviceable-areas

[^13]: https://github.com/iZekCode/E-Commerce-Data-Analysis-SQL-and-Dashboard

[^14]: https://github.com/heiswayi/malaysia-postcodes

[^15]: https://www.triplewhale.com/free
