...
serve(async (req) => {
  ...
  console.log(`Processing redemption for order ${orderId} (customer ${customerId})...`)
  ...
  if (oErr || !order) {
    console.error(`Order lookup failed for ${orderId}:`, oErr)
    return err(oErr?.message || 'Order not found')
  }

  if (order.customer_id !== customerId) {
    console.warn(`Customer mismatch: order.customer_id=${order.customer_id}, body.customerId=${customerId}`)
    return err('Unauthorised')
  }

  if (order.points_redeemed > 0) {
    console.log(`Order ${orderId} already has ${order.points_redeemed} points redeemed.`)
  ...
  if (bErr) {
    console.error(`Balance lookup failed for customer ${customerId}:`, bErr)
  ...
  const finalPoints    = Math.min(actualPoints, pointsToRedeem)
  const newBalance     = currentBalance - finalPoints
  const newTotal       = Math.max(Number(order.total_amount) - actualDiscount, 0)

  console.log(`Requested: ${pointsToRedeem} pts (RM ${discountRM}). Max allowed: RM ${maxDiscountRM}`)
  ...
  if (deductErr) {
    console.error(`Deduction failed for customer ${customerId}:`, deductErr)
  ...
  if (txErr) console.error(`Failed to record redemption transaction for order ${orderId}:`, txErr)
  ...
  if (updateErr) console.error(`Failed to update order ${orderId} totals:`, updateErr)

  console.log(`Successfully redeemed ${finalPoints} pts for order ${orderId}. New total: ${newTotal}`)
  return ok({ 
    success: true,
